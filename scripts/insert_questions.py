...
# 如果使用相对路径的 SQLite，确保指向 backend 目录
...
# ✅ 重新定义 Question 模型（与 main.py 保持一致）
class Question(Base):
...
def insert_questions():
    ...
    try:
        ...
        if existing_count > 0:
            print(f"⚠️  数据库中已有 {existing_count} 个问题")
            choice = input("是否继续添加新问题？(y/n): ").lower()
            if choice != 'y':
                print("❌ 取消导入")
                return
        ...
        db.commit()
        print(f"✅ 成功导入 {added_count} 个问题到数据库！")
        
        # 显示总数
        total_count = db.query(Question).count()
        print(f"📊 数据库中现在共有 {total_count} 个问题")
